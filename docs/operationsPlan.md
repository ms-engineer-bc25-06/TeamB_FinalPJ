...
### 3.1 監視・アラート機能
  - /voice/health エンドポイント（FastAPI）
  - Docker healthcheck（backend, db）
  - /metrics（Prometheus 形式、将来拡張）
  - GitHub Actions 定期監視（30分ごと、失敗時メール通知）

### 3.2 サーバー稼働状況のリアルタイム把握
  - 開発環境: docker ps, docker logs, pg_isready
  - 本番予定: Prometheus + Grafana ダッシュボード

### 3.3 単一障害点（SPOF）の把握
| コンポーネント    | リスク        | 短期対策             | 中期対策           |
| ---------- | ---------- | ---------------- | -------------- |
| PostgreSQL | 単一DB停止     | `pg_dump` バックアップ | レプリカ構成／マネージドDB |
| backend    | 単一コンテナ障害   | Docker自動再起動      | 複数インスタンス＋LB    |
| S3         | リージョン障害    | 週次コピー            | クロスリージョンレプリカ   |
| Firebase   | 単一プロジェクト依存 | APIキー定期更新        | プロジェクト分割       |

### 3.4 データバックアップ計画
  - PostgreSQL: 日次 pg_dump → S3保存（30日保持）、月次リストアテスト
  - S3: 永続保存、週次で別バケットコピー（計画段階）
  - 復元手順: 最新バックアップから新DBを構築 → 接続切替 → 整合性確認

### 3.5 アラート運用
  - 通知経路: GitHub Actions メール通知、Docker 自動再起動ログ
  - 優先度定義: P1=15分以内対応、P2=当日中、P3=翌営業日
  - エスカレーション: P1は即座にバックアップ担当へ、P2/P3は定例レビュー
